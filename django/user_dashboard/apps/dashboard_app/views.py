...
def process_edit(request):
    ...
    if len(errors):
        ...
        User.objects.filter(id=tempid).update(
            first_name=request.POST['edit1'], last_name=request.POST['edit2'], email=request.POST['edit3'])
        return redirect("/home/dashboard")
...
def process_edit_password(request):
    ...
    if len(errors):
        ...
        new_pw_hash = bcrypt.hashpw(
            request.POST['edit4'].encode(), bcrypt.gensalt())
        tempid = request.session['x']
...
def post_message(request):
    my_message = Message.objects.create(message=request.POST['new_message'], poster=User.objects.get(
        id=request.session['id']), recipient=User.objects.get(id=request.session['user_id']))
    return redirect("/home/wall")
...
def post_comment(request):
    if request.method == "POST":
        ...
        my_comment = Comment.objects.create(
            commenter=this_user, commented=this_message, comment=request.POST['new_comment'])
        return redirect("/home/wall")
    return redirect("/home/wall")
